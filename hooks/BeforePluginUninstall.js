...
console.log('linkdev-eclinic-plugin BeforePluginUninstall.js, attempting to modify build.xcconfig');
...
if (lines[headerSearchPathLineNumber].indexOf('linkdev-eclinic-plugin') === -1) {
  console.log('build.xcconfig does not have header path for linkdev-eclinic-plugin');
  return;
}

let line = lines[headerSearchPathLineNumber];
lines[headerSearchPathLineNumber] = line.replace(/\ "\$\(SRCROOT\)\/\$\(PRODUCT_NAME\)\/linkdev-eclinic-plugin\"/i, '');
